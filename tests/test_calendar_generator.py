...
from content_calendar.calendar_generator import (SCOPES,
                                                 ContentCalendarGenerator,
                                                 _validate_client_name,
                                                 _validate_weeks_ahead, main)
...
class TestContentCalendarGenerator:
    ...
    def test_init_with_valid_files(self):
        ...
        with patch(
            "content_calendar.calendar_generator.ContentCalendarGenerator._authenticate"
        ) as mock_auth:
    ...
    def test_init_with_invalid_credentials_path(self):
        ...
        with pytest.raises(
            ValueError, match="Credentials file must be in current directory"
        ):
    ...
    def test_authenticate_with_existing_valid_token(
        self, mock_from_file, mock_exists, mock_authorize
    ):
    ...
    def test_authenticate_with_expired_token_refresh(
        ...
        mock_chmod.assert_called_once_with(
            os.path.basename(self.token_file), stat.S_IRUSR | stat.S_IWUSR
        )

    @patch("gspread.authorize")
    @patch("os.path.exists")
    @patch(
        "content_calendar.calendar_generator.InstalledAppFlow.from_client_secrets_file"
    )
    def test_authenticate_with_new_oauth_flow(
        self, mock_flow_from_file, mock_exists, mock_authorize
    ):
    ...
    @patch(
        "content_calendar.calendar_generator.ContentCalendarGenerator._retry_api_call"
    )
    ...
    def test_set_column_widths_success(self, mock_auth, mock_retry):
        ...
        assert (
            len(call_args[1]["requests"]) == 10
        )  # 10 columns (including automation columns)
    ...
    def test_add_data_validation_failure(self, mock_auth, mock_retry, mock_warning):
        ...
        mock_warning.assert_called_once_with(
            "Could not add data validation: Validation error"
        )

    @patch("logging.info")
    @patch(
        "content_calendar.calendar_generator.ContentCalendarGenerator._create_instructions_sheet"
    )
    @patch(
        "content_calendar.calendar_generator.ContentCalendarGenerator._add_data_validation"
    )
    @patch(
        "content_calendar.calendar_generator.ContentCalendarGenerator._add_sample_data"
    )
    @patch(
        "content_calendar.calendar_generator.ContentCalendarGenerator._set_column_widths"
    )
    @patch(
        "content_calendar.calendar_generator.ContentCalendarGenerator._retry_api_call"
    )
    @patch("content_calendar.calendar_generator.ContentCalendarGenerator._authenticate")
    def test_create_content_calendar_success(
        self,
        mock_auth,
        mock_retry,
        mock_set_widths,
        ...
        mock_info.assert_any_call(
            "Created content calendar: Test Client - Content Calendar"
        )
        mock_info.assert_any_call(
    ...
    def test_create_instructions_sheet(self, mock_auth, mock_retry):
        ...
        mock_retry.assert_any_call(
            mock_spreadsheet.add_worksheet, title="Instructions", rows=50, cols=10
        )
...
class TestMainFunction:
    ...
    def test_main_success(
        ...
        mock_info.assert_any_call(
            "Share this URL with your client: https://docs.google.com/spreadsheets/test"
        )
